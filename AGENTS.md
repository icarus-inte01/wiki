...
> **MkDocs(Material) 렌더링 시 코드 블록과 리스트 사이에는 반드시 빈 줄이 있어야 합니다.**

### 코드 블록 뒤 리스트

코드 블록(` ``` `) 직후에 bullet 리스트(`- `)를 작성하면 MkDocs 렌더러가 리스트로 인식하지 못해 개행 없이 한 줄로 출력됩니다.
...
코드 블록 닫는 ` ``` ` 뒤에 반드시 한 줄을 띄우고 리스트를 시작해야 정상 렌더링됩니다. 다른 블록 요소(테이블, 인용문 등)와 리스트 사이도 동일하게 적용됩니다.
